...
class VoxelQueryAndGrouping(nn.Module):
    ...
    def forward(self, new_coords: torch.Tensor, xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor,
        ...
        idx = idx.unsqueeze_(0) #1,N,sample
        features = features.unsqueeze_(0).permute(0,2,1)
        grouped_features = grouping_operation(features.contiguous(),idx.contiguous()).squeeze(0).permute(1,0,2)  ###
        grouped_features[empty_ball_mask1] = 0
        #grouped_features = pointnet2_utils.grouping_operation(features, xyz_batch_cnt, idx, new_xyz_batch_cnt)  
        
        return grouped_features.contiguous() #grouped_xyz,

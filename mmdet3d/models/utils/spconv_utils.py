...
def scatter_features(indices, features, shape):
    ret = -1 * torch.ones(*shape, dtype=features.dtype, device=features.device)
    ndim = indices.shape[-1]
    flattened_indices = indices.view(-1, ndim)
    slices = [flattened_indices[:, i] for i in range(ndim)]
    ret[slices] = features
...
def generate_voxel2pinds(sparse_tensor,coors2):

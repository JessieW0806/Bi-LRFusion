...
class MVXTwoStageDetector(Base3DDetector):
    ...
    def __init__(self,
        ...
        if pts_voxel_layer2:
            self.pts_voxel_layer2 = Voxelization(**pts_voxel_layer2)
        ...
        self.non_local = NonLocal2D(in_channels=256)
        ...
        #self.deconv = torch.nn.ConvTranspose2d(256, 256, 2, stride=2)
    ...
    def extract_feat(self, points, img, img_metas,radar_points = None):
        """Extract features from images and points."""
        img_feats = self.extract_img_feat(img, img_metas)
        pts_feats = self.extract_pts_feat(points, img_feats, img_metas, radar_points)
    ...
    @torch.no_grad()
    @force_fp32()
    def voxelize2_hard(self, points):
        """Apply dynamic voxelization to points.
        ...
        voxels, coors, num_points = [], [], []
        for res in points:
            res_voxels, res_coors, res_num_points = self.pts_voxel_layer2(res)
            voxels.append(res_voxels)
            coors.append(res_coors)
            num_points.append(res_num_points)
        voxels = torch.cat(voxels, dim=0)
    ...
    def dy_voxelize(self, points):
        ...
        #exit()
        # dynamic voxelization only provide a coors mapping
        #print(self.radar_voxel_layer)
        for res in points:
            #print(res)
            res_coors = self.radar_voxel_layer(res)
            #print(res_coors)
            coors.append(res_coors)
            #print("----")
        points = torch.cat(points, dim=0)
    ...
    def dy_voxelize2(self, points):
        ...
        # dynamic voxelization only provide a coors mapping
        for res in points:
            res_coors = self.radar_voxel_layer2(res)
            coors.append(res_coors)
        points = torch.cat(points, dim=0)
        coors_batch = []
        for i, coor in enumerate(coors):
    ...
    def forward_train(self,
        ...
        img_feats, pts_feats = self.extract_feat(
            points, img=img, img_metas=img_metas,radar_points = radar_points)
        losses = dict()
        if pts_feats:
    ...
    def simple_test(self, points, img_metas, img=None, rescale=False,radar_points=None):
        """Test function without augmentaiton."""
        img_feats, pts_feats = self.extract_feat(
            points, img=img, img_metas=img_metas,radar_points=radar_points[0])

        bbox_list = [dict() for i in range(len(img_metas))]
        if pts_feats and self.with_pts_bbox:
            bbox_pts = self.simple_test_pts(
                pts_feats, img_metas, rescale=rescale,radar_points=radar_points)
    ...
    def extract_feats(self, points, img_metas, imgs=None, radar_points=[None]*4):

...
class LiDARInstance3DBoxes(BaseInstance3DBoxes):
    ...
    def rotate(self, angle, points=None, radar_points=None):
        ...
        if points is not None:
            ...
            if radar_points is not None:
                if isinstance(radar_points, torch.Tensor):
                    radar_points[:, :3] = radar_points[:, :3] @ rot_mat_T
                elif isinstance(radar_points, np.ndarray):
                    rot_mat_T = rot_mat_T.numpy()
                    radar_points[:, :3] = np.dot(radar_points[:, :3], rot_mat_T)
                elif isinstance(radar_points, BasePoints):
                    # clockwise
                    radar_points.rotate(-angle)
                else:
                    raise ValueError
            
            return points, rot_mat_T, radar_points


    def flip(self, bev_direction='horizontal', points=None, radar_points=None):
        ...
        if points is not None:
            ...
            if radar_points is not None:
                assert isinstance(radar_points, (torch.Tensor, np.ndarray, BasePoints))
                if isinstance(radar_points, (torch.Tensor, np.ndarray)):
                    if bev_direction == 'horizontal':
                        radar_points[:, 1] = -radar_points[:, 1]
                    elif bev_direction == 'vertical':
                        radar_points[:, 0] = -radar_points[:, 0]
                elif isinstance(radar_points, BasePoints):
                    radar_points.flip(bev_direction)

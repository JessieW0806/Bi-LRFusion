    ...
    def random_flip_data_3d(self, input_dict, direction='horizontal'):
        ...
        for key in input_dict['bbox3d_fields']:
            if 'points' in input_dict:
                input_dict['points'], radar_points = input_dict[key].flip(
                    direction, points=input_dict['points'], radar_points=input_dict.get('radar_points',None))
...
class GlobalRotScaleTrans(object):
    ...
    def _rot_bbox_points(self, input_dict):
        ...
        if len(input_dict['bbox3d_fields']) == 0:
            ...
            if 'radar_points' in input_dict:
                rad_rotmatT = input_dict['points'].rotate(noise_rotation)
        ...
        for key in input_dict['bbox3d_fields']:
            if len(input_dict[key].tensor) != 0:
                points, rot_mat_T, radar_points = input_dict[key].rotate(
                    noise_rotation, input_dict['points'], input_dict.get('radar_points',None))
                input_dict['points'] = points
                input_dict['pcd_rotation'] = rot_mat_T
                if 'radar_points' in input_dict:
                    input_dict['radar_points'] = radar_points
...
class PointShuffle(object):
    ...
    def __call__(self, input_dict):
        ...
        idx = idx.numpy()
        if 'radar_points' in input_dict:
            rad_idx = input_dict['radar_points'].shuffle()
...
class PointsRangeFilter(object):
    ...
    def __init__(self, point_cloud_range, radar_cloud_range=[-51.2, -51.2, -5.0, 51.2, 51.2, 3.0]):
        self.pcd_range = np.array(point_cloud_range, dtype=np.float32)
        self.rad_range = np.array(radar_cloud_range, dtype=np.float32)
    ...
    def __call__(self, input_dict):
        ...
        if 'radar_points' in input_dict:
            radar_points = input_dict['radar_points'].tensor
            in_range_flags = ((radar_points[:, 0] > self.rad_range[0])
                          & (radar_points[:, 1] > self.rad_range[1])
                          & (radar_points[:, 0] < self.rad_range[3])
                          & (radar_points[:, 1] < self.rad_range[4]))
            input_dict['radar_points'] = input_dict['radar_points'][in_range_flags]
        pts_instance_mask = input_dict.get('pts_instance_mask', None)
        pts_semantic_mask = input_dict.get('pts_semantic_mask', None)

...
from nuscenes.nuscenes import NuScenes  #ly
...
import torch
#from mmdet3d.datasets.nuscenes_utils import PointCloud_gpu, view_points_gpu

####need to change!   set it in config
nu_version = 'v1.0-trainval'
#nu_version = 'v1.0-mini'
renusc = NuScenes(version=nu_version, dataroot=str('data/nuscenes/'+nu_version), verbose=True) #ly
...
def get_nu_radar(sam_idx, mutil_sweep=True, dims=6, filter=True):
    sample_data = renusc.get('sample', sam_idx)
    ...
    for token in datas.keys():  #每个mm波雷达
        ...
        if mutil_sweep:
            if filter:
                pc, times = RadarPointCloud.from_file_multisweep(renusc,
